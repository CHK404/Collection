...
namespace WindowsFormsApp_14_Collection
{
    public partial class Form1 : Form
    {
        public Form1()
        {
            ...
            //일반 배열 예시
            string[] fruits = new string[2];    //배열 선언시 크기 고정
            fruits[0] = "사과";
            fruits[1] = "바나나";
             ...
             * ㄴ int, string, double 등 어떤 값을 넣어도 자동으로 object로 Boxing
             * ㄴ 어떤 자료형이든 추가할 수 있지만 데이터를 꺼내어 사용시 형변환 필요
             * - 옛날 방식, C# 최신 개발에서든 거의 사용하지 않음
             * 
             * 안쓰는 이유
             * 1) 타입 안정성 부족 - object 타입으로 저장되므로 잘못된 형변환 런타임 오류 가능성 증가
             * 2) 성능 저하 - Boxing, UnBoxing 발생으로 메모리 사용
             * 3) 유지보수 어려움 - 요소 타입이 명확하지 않아 실수 잦음
             * 
             * 용어 정리
             * - Boxing: 값 타입 -> 참조 타입인 object로 변환하는 과정
             * - UnBoxing: Boxing된 object를 다시 원래 값 타입으로 꺼내는 과정
             */
            ...
            //ex Sort, Reverse
            //- 같은 타입만 있는 경우에 가능(다른 타입 존재 시 오류)
            ...
            //데이터 반복 출력
            //DictionaryEntry
            //ㄴ Hashtable의 한 요소를 나타내는 Key-Value 쌍 구조체
            //ㄴ Key, Value 값을 따로 가져올 수 있음
            Console.WriteLine("\n전체항목: ");
             ...
             * ㄴ 일반적으로 <T>, <TKey, TValue> 형태로 사용
             * ㄴ 컴파일 시점에 타입이 결정되어 형변환 없이 안전하게 데이터 처리 가능
             * 
             * 쓰는 이유
             * 1) 타입 안정성 향상 - 런타임 오류 감소
             * 2) 성능 향상 - Boxing/UnBoxing x -> 속도 향상, 메모리 사용 감소
             * 3) 가독성과 유지보수 향상 - 어떤 타입의 값이 저장되는지 명확 -> 실수 방지
             * 
             * 용어 정리
             ...
             * 
             * 주의
             * - 제네릭 컬렉션은 특정 타입에만 제한되어 여러 타입을 저장하려면 구조체나 클래스등으로 묶어야
             * 
            ...
            //5. 요소 제거
            //numbers.Remove(20); //20 제거
            //numbers.RemoveAt(0);    //0번 인덱스의 요소 제거
            ...
            //7. 기타 메서드
            Console.WriteLine($"Count: {numbers.Count}");   //요소 갯수
            Console.WriteLine($"Contains 15? {numbers.Contains(15)}");  //15가 존재하는지
            ...
            Console.WriteLine("[현재 Queue 상태 재확인]");
            foreach (string item in queue2)
            {
                Console.WriteLine(item);
            }
            ...
            //ㄴ 내부적으로 Hashtable 구조를 사용해서 검색 속도 매우 빠름
            Console.WriteLine("\r\n");
            ...
            //인스턴스를 만들거나 메서드를 호출 할 때 실제 타입으로 결정
            Console.WriteLine("\r\n=====List<T>=====");
            ...
            {
                Console.WriteLine($"민수의 점수: {scores2}");
            }
